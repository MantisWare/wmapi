      ...
      if (_.get(this._attributes, `${current}.isVirtual`) !== true && _.isUndefined(association)) {
        ...
        switch (association.nature) {
          ...
          case 'oneToOne':
            if (response[current] !== params.values[current]) {
              ...
              if (response[current] && _.isObject(response[current]) && response[current][this.primaryKey] !== value[current]) {
                virtualFields.push(
                  mwapi.query(details.collection || details.model, details.plugin).update({
              ...
              virtualFields.push(
                mwapi.query(details.model || details.collection, details.plugin).findOne({ id : recordId })
                  .then(record => {
              ...
              virtualFields.push(mwapi.query(details.model || details.collection, details.plugin).update({
                id: recordId,
                values: {
          ...
          case 'manyToMany':
            if (details.dominant === true) {
              ...
              toAdd.forEach(value => {
                ...
                virtualFields.push(mwapi.query(details.model || details.collection, details.plugin).addRelation({
              ...
              toRemove.forEach(value => {
                ...
                virtualFields.push(mwapi.query(details.model || details.collection, details.plugin).removeRelation({
                  id: value[this.primaryKey] || value.id || value._id,
                  values: value,
                  foreignKey: current
                }));
          ...
          case 'manyMorphToOne':
            ...
            acc[current] = params.values[current].map(obj => {
              const globalId = obj.source && obj.source !== 'content-manager' ?
                mwapi.plugins[obj.source].models[_.toLower(obj.ref)].globalId:
                mwapi.models[_.toLower(obj.ref)].globalId;
          ...
          case 'manyToManyMorph':
            ...
            toAdd.forEach(id => {
              virtualFields.push(mwapi.query(details.model || details.collection, details.plugin).addRelationMorph({
                id,
            ...
            // Remove relations in the other side.
            toRemove.forEach(id => {
              virtualFields.push(mwapi.query(details.model || details.collection, details.plugin).removeRelationMorph({
                id,
                alias: association.via,
                ref: this.globalId,
                refId: response._id,
