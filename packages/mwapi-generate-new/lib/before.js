...
module.exports = (scope, cb) => {
  ...
  _.defaults(scope, {
    ...
    author: process.env.USER || 'A mwapi developer',
  ...
  // Make changes to the rootPath where the mwapi project will be created.
  ...
  try {
    ...
    if (files.length > 1) {
      return logger.error('`$ mwapi new` can only be called in an empty directory.');
    }
  } catch (err) {
    // ...
  }
          ...
          .prompt([
            {
              ...
              default: _.get(scope.database, 'database', 'mwapi')
            },
            {
              when: !hasDatabaseConfig,
              type: 'input',
