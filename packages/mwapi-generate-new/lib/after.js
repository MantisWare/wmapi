...
module.exports = (scope, cb) => {
  ...
  const mwapiRootPath = path.resolve(scope.mwapiRoot, '..');
  ...
  const mwapiDependencies = Object.keys(dependencies).filter(key => key.indexOf('mwapi') !== -1 && key.indexOf('mwapi-bookshelf') === -1);
  const othersDependencies = Object.keys(dependencies).filter(key => key.indexOf('mwapi') === -1  || key.indexOf('mwapi-bookshelf') !== -1);

  // Verify if the dependencies are available into the global
  _.forEach(mwapiDependencies, (key) => {
    try {
  ...
  const mwapiBin = path.join(scope.mwapiRoot, scope.mwapiPackageJSON.bin.mwapi);
  ...
  function pluginsInstallation() {
    ...
    defaultPlugins.forEach(defaultPlugin => {
      try {
        execSync(`node ${mwapiBin} install ${defaultPlugin.name} ${scope.developerMode && defaultPlugin.core ? '--dev' : ''}`);
    ...
    availableDependencies.forEach(dependency => {
      ...
      if (dependency.global) {
        ...
        try {
          fs.accessSync(path.resolve(scope.mwapiRoot, 'node_modules', dependency.key), fs.constants.W_OK | fs.constants.F_OK);
          fs.symlinkSync(path.resolve(scope.mwapiRoot, 'node_modules', dependency.key), path.resolve(scope.rootPath, 'node_modules', dependency.key), 'dir');
        } catch (e) {
          // Silent.
        }
      }
