...
module.exports = mwapi => {
  const hook = {
    ...
    initialize: cb => {
      ...
      _.forEach(_.pickBy(mwapi.config.connections, {connector: 'mwapi-bookshelf'}), (connection, name) => {
        ...
        if (!_.includes(CLIENTS, connection.settings.client)) {
          mwapi.log.error('The client `' + connection.settings.client + '` for the `' + name + '` connection is not supported.');
          mwapi.stop();
        }
        ...
        try {
          require(path.resolve(mwapi.config.appPath, 'node_modules', connection.settings.client));
        } catch (err) {
          mwapi.log.error('The client `' + connection.settings.client + '` is not installed.');
          mwapi.log.error('You can install it with `$ npm install ' + connection.settings.client + ' --save`.');
          mwapi.stop();
        }
        ...
        }, mwapi.config.hook.settings.knex);
        ...
        try {
          ...
          _.set(mwapi, `connections.${name}`, require(path.resolve(mwapi.config.appPath, 'node_modules', 'knex'))(options));
        } catch (err) {
          mwapi.log.error('Impossible to use the `' + name + '` connection...');
          mwapi.log.warn('Be sure that your client `' + name + '` are in the same node_modules directory');
          mwapi.log.error(err);
          mwapi.stop();
        }
      });
