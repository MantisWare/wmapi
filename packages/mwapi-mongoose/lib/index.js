...
const { models: utilsModels, logger }  = require('mwapi-utils');
...
module.exports = function (mwapi) {
  const hook = {
    ...
    initialize: cb => {
      _.forEach(_.pickBy(mwapi.config.connections, {connector: 'mwapi-mongoose'}), (connection, connectionName) => {
        const instance = new Mongoose();
        const { uri, host, port, username, password, database } = _.defaults(connection.settings, mwapi.config.hook.settings.mongoose);
          ...
          const mountModels = (models, target, plugin = false) => {
            ...
            const loadedAttributes = _.after(_.size(models), () => {
              _.forEach(models, (definition, model) => {
                try {
                  let collection = mwapi.config.hook.settings.mongoose.collections[mongooseUtils.toCollectionName(definition.globalName)];
                  ...
                  */
                  ...
                  _.forEach(preLifecycle, (fn, key) => {
                    if (_.isFunction(target[model.toLowerCase()][fn])) {
                      collection.schema.pre(key, function (next) {
                        target[model.toLowerCase()][fn](this).then(next).catch(err => mwapi.log.error(err));
                      });
                    }
                  });
                  ...
                  _.forEach(postLifecycle, (fn, key) => {
                    if (_.isFunction(target[model.toLowerCase()][fn])) {
                      collection.schema.post(key, function (doc, next) {
                        target[model.toLowerCase()][fn](this, doc).then(next).catch(err => mwapi.log.error(err))
                      });
                    }
                  });
                  ...
                  // Push model to mwapi global variables.
            ...
            _.forEach(models, (definition, model) => {
              ...
              if (_.isEmpty(definition.connection)) {
                definition.connection = mwapi.config.currentEnvironment.database.defaultConnection;
              }

              // Make sure this connection exists.
              if (!_.has(mwapi.config.connections, definition.connection)) {
                mwapi.log.error('The connection `' + definition.connection + '` specified in the `' + model + '` model does not exist.');
                mwapi.stop();
              }

              // Add some informations about ORM & client connection
              definition.orm = 'mongoose';
              definition.client = _.get(mwapi.config.connections[definition.connection], 'client');
              ...
              if (_.isEmpty(definition.attributes)) {
                ...
                _.set(mwapi.config.hook.settings.mongoose, 'collections.' + mongooseUtils.toCollectionName(definition.globalName) + '.schema', new instance.Schema({}));
              ...
              const done = _.after(_.size(definition.attributes), () => {
                ...
                _.set(mwapi.config.hook.settings.mongoose, 'collections.' + mongooseUtils.toCollectionName(definition.globalName) + '.schema', schema);
              ...
              _.forEach(definition.attributes, (details, name) => {
                ...
                switch (verbose) {
                  case 'hasOne': {
                    const ref = details.plugin ? mwapi.plugins[details.plugin].models[details.model].globalId : mwapi.models[details.model].globalId;
                  ...
                  case 'hasMany': {
                    ...
                    const ref = details.plugin ? mwapi.plugins[details.plugin].models[details.collection].globalId : mwapi.models[details.collection].globalId;
            ...
            case 'manyMorphToOne':
              ...
              acc[current] = params.values[current].map(obj => {
                const globalId = obj.source && obj.source !== 'content-manager' ?
                  mwapi.plugins[obj.source].models[obj.ref].globalId:
                  mwapi.models[obj.ref].globalId;

                // Define the object stored in database.
                // The shape is this object is defined by the mwapi-mongoose connector.
                return {
                  ref: obj.refId,
                  kind: globalId,
