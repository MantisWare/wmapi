...
export function* getTableExistance() {
  try {
    ...
    mwapi.notification.error('notification.error');
  }
}
  ...
  try {
    ...
    if (response.ok) {
      if (method === 'POST') {
        ...
        mwapi.notification.success('content-type-builder.notification.success.message.contentType.create');

      } else {
        mwapi.notification.success('content-type-builder.notification.success.message.contentType.edit');
      }
    ...
    mwapi.notification.error(get(error, ['response', 'payload', 'message', '0', 'messages', '0', 'id'], 'notification.error'));
    yield put(unsetButtonLoader());
  }
}
