...
export class ModelPage extends React.Component { // eslint-disable-line react/prefer-stateless-function
  ...
  handleAddLinkClick = () => {
    if (storeData.getIsModelTemporary()) {
      mwapi.notification.info('content-type-builder.notification.info.contentType.creating.notSaved');
    } else {
      this.toggleModal();
    }
  }
  ...
  handleEditAttribute = (attributeName) => {
    ...
    if (attribute.params.type === 'enumeration') {
      return mwapi.notification.info('content-type-builder.notification.info.enumeration');
    }
