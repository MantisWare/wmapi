...
// mwapi helpers for models.
const utilsModels = require('mwapi-utils').models;
...
module.exports = function(mwapi) {
  const hook = {
    ...
    initialize: cb => {
      const connections = _.pickBy(mwapi.config.connections, { connector: 'mwapi-bookshelf' });

      _.forEach(connections, (connection, connectionName) => {
        // Apply defaults
        _.defaults(connection.settings, mwapi.config.hook.settings.bookshelf);

        // Create Bookshelf instance for this connection.
        const ORM = new bookshelf(mwapi.connections[connectionName]);
        ...
        try {
          ...
          require(path.resolve(
            ...
            'functions',
            'bookshelf.js'
          ))(ORM, mwapi.connections[connectionName]);
        } catch (err) {
          ...
          _.forEach(models, (definition, model) => {
            ...
            const done = _.after(_.size(definition.attributes), () => {
              try {
                  ...
                  polymorphicAssociations.map(association => {
                    ...
                    if (relation) {
                      ...
                      const model = association.plugin ?
                        mwapi.plugins[association.plugin].models[association.collection || association.model]:
                        mwapi.models[association.collection || association.model];
                ...
                // Expose ORM functions through the `mwapi.models[xxx]`
                // or `mwapi.plugins[xxx].models[yyy]` object.
                ...
                mwapi.log.error('Impossible to register the `' + model + '` model.');
                mwapi.log.error(err);
                mwapi.stop();
              }
            });
            ...
            _.forEach(definition.attributes, (details, name) => {
              ...
              if (globalName !== '*') {
                globalId = details.plugin ?
                  _.get(mwapi.plugins,`${details.plugin}.models.${globalName.toLowerCase()}.globalId`):
                  _.get(mwapi.models, `${globalName.toLowerCase()}.globalId`);
              }
              ...
              switch (verbose) {
                case 'hasOne': {
                  const FK = details.plugin ?
                    _.findKey(
                      mwapi.plugins[details.plugin].models[details.model].attributes,
                      details => {
                        if (
                    ...
                    _.findKey(
                      mwapi.models[details.model].attributes,
                      details => {
                        if (
                          details.hasOwnProperty('model') &&
                          details.model === model &&
                  ...
                  const columnName = details.plugin ?
                    _.get(mwapi.plugins, `${details.plugin}.models.${details.model}.attributes.${FK}.columnName`, FK):
                    _.get(mwapi.models, `${details.model}.attributes.${FK}.columnName`, FK);
                ...
                case 'hasMany': {
                  const columnName = details.plugin ?
                    _.get(mwapi.plugins, `${details.plugin}.models.${globalId.toLowerCase()}.attributes.${details.via}.columnName`, details.via):
                    _.get(mwapi.models[globalId.toLowerCase()].attributes, `${details.via}.columnName`, details.via);
                ...
                case 'belongsToMany': {
                  const collection = details.plugin ?
                    mwapi.plugins[details.plugin].models[details.collection]:
                    mwapi.models[details.collection];
                  ...
                  // Define PK column
                  details.column = utils.getPK(model, mwapi.models);
                ...
                case 'morphOne': {
                  const model = details.plugin ?
                    mwapi.plugins[details.plugin].models[details.model]:
                    mwapi.models[details.model];
                ...
                case 'belongsToManyMorph': {
                  ...
                  const morphValues = association.related.map(id => {
                    let models = Object.values(mwapi.models).filter(model => model.globalId === id);

                    if (models.length === 0) {
                      models = Object.keys(mwapi.plugins).reduce((acc, current) => {
                        const models = Object.values(mwapi.plugins[current].models).filter(model => model.globalId === id);
                    ...
                    if (models.length === 0) {
                      mwapi.log.error('Impossible to register the `' + model + '` model.');
                      mwapi.log.error('The collection name cannot be found for the morphTo method.');
                      mwapi.stop();
                    }
        ...
        // Mount `./api` models.
        mountModels(_.pickBy(mwapi.models, { connection: connectionName }), mwapi.models);

        // Mount `./plugins` models.
        _.forEach(mwapi.plugins, (plugin, name) => {
          mountModels(_.pickBy(mwapi.plugins[name].models, { connection: connectionName }), plugin.models, name);
        });
      });
    ...
    manageRelations: async function (model, params) {
      const models = _.assign(_.clone(mwapi.models), Object.keys(mwapi.plugins).reduce((acc, current) => {
        _.assign(acc, mwapi.plugins[current].models);
        return acc;
      }, {}));
            ...
            case 'manyMorphToOne':
              ...
              params.values[current].forEach(obj => {
                const model = obj.source && obj.source !== 'content-manager' ?
                  mwapi.plugins[obj.source].models[obj.ref]:
                  mwapi.models[obj.ref];
