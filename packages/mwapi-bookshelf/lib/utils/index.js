...
module.exports = {
  ...
  getPK: (collectionIdentity, collection, models) => {
    ...
    try {
      ...
      if (typeof mwapi !== 'undefined') {
        collection = _.get(mwapi, `bookshelf.collections.${collectionIdentity}`);
      }
  ...
  getCount: type => {
    return mwapi.bookshelf.collections[type].forge().count().then(count => count);
  }
};
