    ...
    return new Promise((resolve, reject) => {
      ...
      this.store = (source = {}) => {
        const get = async (params = {}) => {
          ...
          const findAction = mwapi.models['core_store'].orm === 'mongoose' ? 'findOne' : 'forge';
          ...
          const data = mwapi.models['core_store'].orm === 'mongoose'
          ? await mwapi.models['core_store'].findOne(where)
          : await mwapi.models['core_store'].forge(where).fetch().then(config => {
            if (config) {
        ...
        const set = async (params = {}) => {
          ...
          let data = mwapi.models['core_store'].orm === 'mongoose'
          ? await mwapi.models['core_store'].findOne(where)
          : await mwapi.models['core_store'].forge(where).fetch().then(config => {
            if (config) {
              return config.toJSON();
          ...
          if (data) {
            ...
            mwapi.models['core_store'].orm === 'mongoose'
            ? await mwapi.models['core_store'].update({ _id: data._id }, data, { strict: false })
            : await mwapi.models['core_store'].forge({ id: data.id }).save(data, { patch: true });
          } else {
            Object.assign(where, {
              value: JSON.stringify(value) || value.toString(),
              type: (typeof value).toString(),
            ...
            mwapi.models['core_store'].orm === 'mongoose'
            ? await mwapi.models['core_store'].create(where)
            : await mwapi.models['core_store'].forge().save(where);
          }
        };
