...
const { logger, models } = require('mwapi-utils');
...
/**
 * Construct an mwapi instance.
 *
 * @constructor
 */

class mwapi extends EventEmitter {
  constructor() {
    super();
  ...
  async start(config = {}, cb) {
    try {
      ...
      this.server.listen(this.config.port, err => {
        ...
        this.log.debug(`Version: ${this.config.info.mwapi} (node v${this.config.info.node})`);
  ...
  async load() {
    this.app.use(async (ctx, next) => {
      if (ctx.request.url === '/_health' && ctx.request.method === 'HEAD') {
        ctx.set('mwapi', 'You are so French !');
        ctx.status = 204;
      } else {
        await next();
      }
  ...
  async bootstrap() {
    const execBootstrap = (fn) => !fn ? Promise.resolve() : new Promise((resolve, reject) => {
      ...
      try {
        fn(err => {
          if (ranBootstrapFn) {
            this.log.error('You called the callback in `mwapi.config.boostrap` more than once!');
  ...
  query(entity, plugin) {
    ...
    const Model = get(mwapi.plugins, [plugin, 'models', model]) || get(mwapi, ['models', model]) || undefined;
    ...
    if (file.indexOf('mwapi-plugin-') !== -1) {
      pluginPath = file.split(path.sep).filter(x => x.indexOf('mwapi-plugin-') !== -1)[0];
    ...
    if (!pluginPath) {
      return this.log.error('Impossible to find the plugin where `mwapi.query` has been called.');
    }

    // Get plugin name.
    const pluginName = pluginPath.replace('mwapi-plugin-', '').toLowerCase();
