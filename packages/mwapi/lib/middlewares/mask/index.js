...
module.exports = mwapi => {
  return {
    ...
    initialize: function (cb) {
      ...
      const enabled = (() => {
        const main = Object.keys(mwapi.models).reduce((acc, current) => {
          if (Object.values(mwapi.models[current].attributes).find(attr => attr.private === true)) {
            acc = true;
          }
        ...
        const plugins = Object.keys(mwapi.plugins).reduce((acc, plugin) => {
          const bool = Object.keys(mwapi.plugins[plugin].models).reduce((acc, model) => {
            if (Object.values(mwapi.plugins[plugin].models[model].attributes).find(attr => attr.private === true)) {
              acc = true;
            }
    ...
    mask: function (ctx, value) {
      ...
      const attributesToHide = models.reduce((acc, match) => {
        const attributes = match.plugin ?
          mwapi.plugins[match.plugin].models[match.model].attributes:
          mwapi.models[match.model].attributes;
    ...
    whichModels: function (value, plugin) {
      ...
      const match = (model, plugin) => {
        const attributes = plugin ?
          Object.keys(mwapi.plugins[plugin].models[model].attributes):
          Object.keys(mwapi.models[model].attributes);
      ...
      // Application models.
      Object.keys(mwapi.models).forEach(model => match(model));
      // Plugins models.
      Object.keys(mwapi.plugins).forEach(plugin => {
        Object.keys(mwapi.plugins[plugin].models).forEach(model => match(model, plugin));
      });
