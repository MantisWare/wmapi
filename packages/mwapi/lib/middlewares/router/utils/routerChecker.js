...
// mwapi utilities.
const finder = require('mwapi-utils').finder;
const regex = require('mwapi-utils').regex;
const joijson = require('mwapi-utils').joijson;
...
module.exports = mwapi => function routerChecker(value, endpoint, plugin) {
  const Joi = mwapi.koaMiddlewares.routerJoi.Joi;
  ...
  const controller = plugin
    ? mwapi.plugins[plugin].controllers[handler[0].toLowerCase()]
    : mwapi.controllers[handler[0].toLowerCase()] ||
        mwapi.admin.controllers[handler[0].toLowerCase()];

  const action = controller[handler[1]];

  // Retrieve the API's name where the controller is located
  // to access to the right validators
  const currentApiName = finder(mwapi.plugins[plugin] || mwapi.api || mwapi.admin, controller);
