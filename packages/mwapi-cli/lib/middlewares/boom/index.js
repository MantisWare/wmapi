...
module.exports = mwapi => {
  return {
    ...
    initialize: function(cb) {
      this.delegator = delegate(mwapi.app.context, 'response');
    ...
    createResponses: function() {
      Object.keys(Boom).forEach(key => {
        mwapi.app.response[key] = function(...rest) {
      ...
      mwapi.app.response.send = function(data) {
        this.status = 200;
        this.body = data;
      };

      mwapi.app.response.created = function(data) {
        this.status = 201;
        this.body = data;
      };
