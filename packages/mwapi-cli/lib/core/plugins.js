...
module.exports = function() {
  return new Promise((resolve, reject) => {
    const folder = ((url = _.get(mwapi.config.currentEnvironment.server, 'admin.path', 'admin')) =>
    ...
    const sourcePath = process.env.NODE_ENV !== 'test' ?
      ...
      path.resolve(this.config.appPath, 'packages', 'mwapi-admin', 'admin', 'src', 'config', 'plugins.json');
    const buildPath = process.env.NODE_ENV !== 'test' ?
      path.resolve(this.config.appPath, 'admin', 'admin', 'build', 'config', 'plugins.json'):
      path.resolve(this.config.appPath, 'packages', 'mwapi-admin', 'admin', 'build', 'config', 'plugins.json');
