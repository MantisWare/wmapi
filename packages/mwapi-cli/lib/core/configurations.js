...
module.exports.app = async function() {
    // Retrieve mwapi version.
    this.config.uuid = get(this.config.info, 'mwapi.uuid', '');
    this.config.info.mwapi = (get(this.config, 'info.dependencies.mwapi') || '').replace(/(\^|~)/g, ''),
    ...
    if (get(this.config, 'language.enabled')) {
      this.config.language.locales = Object.keys(get(mwapi.config, 'locales', {}));
    }
    ...
    for (let name in this.config.connections) {
      ...
      const connector = connection.connector.replace('mwapi-', '');
...
const enableHookNestedDependencies = function (name, flattenHooksConfig, force = false) {
  if (!this.hook[name]) {
    this.log.warn(`(hook:${name}) \`mwapi-${name}\` is missing in your dependencies. Please run \`npm install mwapi-${name}\``);
  }
  ...
  if (isEmpty(get(flattenHooksConfig, name, true))) {
    ...
    const modelsUsed = Object.keys(assign(clone(this.api) || {}, this.plugins))
      ...
      .filter(models => {
        const apiModelsUsed = Object.keys(models).filter(model => {
          ...
          if (connector) {
            return connector.replace('mwapi-', '') === name;
          }
    ...
    if (get(this.hook, `${name}.dependencies`, []).length > 0) {
      this.hook[name].dependencies.forEach(dependency => {
        enableHookNestedDependencies.call(this, dependency.replace('mwapi-', ''), flattenHooksConfig, true);
      });
    }
  }
};
