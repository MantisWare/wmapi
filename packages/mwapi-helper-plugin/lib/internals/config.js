...
const mwapiPlugin = {
  ...
  dllPlugin: {
    defaults: {
      ...
      path: resolve('node_modules', 'mwapi-plugin-dlls'),
    },
    ...
    entry(helperPkg, pluginPkg) {
      ...
      const exclude = pluginPkg.dllPlugin.exclude || mwapiPlugin.dllPlugin.defaults.exclude;
      const include = pluginPkg.dllPlugin.include || mwapiPlugin.dllPlugin.defaults.include;
      const includeDependencies = uniq(dependencyNames.concat(include));

      return {
        mwapiPluginDeps: pullAll(includeDependencies, exclude),
      };
    },
  },
};

module.exports = mwapiPlugin;
