...
const pluginId = pkg.name.replace(/^mwapi-/i, '');
...
const adminPath = (() => {
  if (isAdmin && isSetup) {
    return path.resolve(appPath, 'mwapi-admin');
  }
...
if (!isSetup) {
  try {
    ...
    const mwapi = require(path.join(appPath, 'node_modules', 'mwapi'));

    mwapi.config.appPath = appPath;
    ...
    console.log(e);
    throw new Error(`You need to start the WebPack server from the /admin or /plugins/**/admin directories in a mwapi's project.`);
  }
}
...
if (process.env.npm_lifecycle_event === 'start') {
  ...
  plugins.folders = plugins.src.reduce((acc, current) => {
    acc[current] = path.resolve(appPath, 'plugins', current, 'node_modules', 'mwapi-helper-plugin', 'lib', 'src');
  ...
  module: {
    loaders: [{
      ...
      include: [path.join(adminPath, 'admin', 'src')]
        ...
        .concat([path.join(adminPath, 'node_modules', 'mwapi-helper-plugin', 'lib', 'src')])
    }, {
