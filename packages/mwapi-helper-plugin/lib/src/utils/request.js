...
function checkTokenValidity(response) {
  ...
  if (auth.getToken()) {
    return fetch(`${mwapi.backendURL}/user/me`, options)
    .then(resp => {
      if (response.status === 401) {
        window.location = `${mwapi.remoteURL}/plugins/users-permissions/auth/login`;
...
function serverRestartWatcher(response) {
  return new Promise((resolve, reject) => {
    fetch(`${mwapi.backendURL}/_health`, {
      method: 'HEAD',
      mode: 'no-cors',
      headers: {
        'Content-Type': 'application/json',
...
export default function request(url, options = {}, shouldWatchServerRestart = false, stringify = true ) {
 // Set headers
  ...
  url = _.startsWith(url, '/') ? `${mwapi.backendURL}${url}` : url;
