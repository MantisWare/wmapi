...
exports.connect = (provider, query) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    getProfile(provider, query, async (err, profile) => {
      ...
      try {
        const users = await mwapi.query('user', 'users-permissions').find({
          email: profile.email
        });

        const advanced = await mwapi.store({
          environment: '',
          type: 'plugin',
        ...
        // Retrieve role `guest`.
        const guest = await mwapi.query('role', 'users-permissions').findOne({ type: 'guest' }, []);
        ...
        const createdUser = await mwapi.query('user', 'users-permissions').create(params);
