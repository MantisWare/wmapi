...
module.exports = {
  createRole: async (params) => {
    ...
    const role = await mwapi.query('role', 'users-permissions').create(_.omit(params, ['users', 'permissions']));
    ...
    const arrayOfPromises = Object.keys(params.permissions).reduce((acc, type) => {
      Object.keys(params.permissions[type].controllers).forEach(controller => {
        Object.keys(params.permissions[type].controllers[controller]).forEach(action => {
          acc.push(mwapi.query('permission', 'users-permissions').addPermission({
    ...
    arrayOfPromises.push(mwapi.plugins['content-manager'].services['contentmanager'].edit({
      id: role._id || role.id,
      model: 'role'
    }, {
      users: params.users
  ...
  deleteRole: async (roleID, guestID) => {
    const role = await mwapi.query('role', 'users-permissions').findOne({ id: roleID }, ['users', 'permissions']);
    ...
    const arrayOfPromises = role.users.reduce((acc, user) => {
      acc.push(mwapi.query('user', 'users-permissions').update({
        id: user._id || user.id
      }, {
        role: guestID
      }))
    ...
    role.permissions.forEach(permission => {
      arrayOfPromises.push(mwapi.query('permission', 'users-permissions').delete({
        id: permission._id || permission.id
      }));
    })

    // Delete the role.
    arrayOfPromises.push(mwapi.query('role', 'users-permissions').delete({
      id: roleID
    }));
  ...
  getActions: (plugins = [], withInfo = true) => {
    ...
    const appControllers = Object.keys(mwapi.api || {}).reduce((acc, key) => {
      acc.controllers[key] = generateActions(mwapi.api[key].controllers[key]);

      return acc;
    }, { controllers: {} });

    const pluginsPermissions = Object.keys(mwapi.plugins).reduce((acc, key) => {
      const initialState = {
        controllers: {}
      };
      ...
      acc[key] = Object.keys(mwapi.plugins[key].controllers).reduce((obj, k) => {
        obj.controllers[k] = generateActions(mwapi.plugins[key].controllers[k]);
  ...
  getRoles: async () => {
    const roles = await mwapi.query('role', 'users-permissions').find({ sort: 'name ASC' }, []);

    for (let i = 0; i < roles.length; ++i) {
      roles[i].id = roles[i].id || roles[i]._id;
      roles[i].nb_users = await mwapi.query('user', 'users-permissions').count({ role: roles[i].id });
    }
  ...
  getRoutes: async () => {
    const routes = Object.keys(mwapi.api || {}).reduce((acc, current) => {
      return acc.concat(mwapi.api[current].config.routes);
    }, []);

    const pluginsRoutes = Object.keys(mwapi.plugins || {}).reduce((acc, current) => {
      acc[current] = mwapi.plugins[current].config.routes;
  ...
  updatePermissions: async function (cb) {
    const actions = mwapi.plugins['users-permissions'].config.actions || [];

    // Aggregate first level actions.
    const appActions = Object.keys(mwapi.api || {}).reduce((acc, api) => {
      Object.keys(mwapi.api[api].controllers)
        .map(controller => {
          const actions = Object.keys(mwapi.api[api].controllers[controller])
            .filter(action => _.isFunction(mwapi.api[api].controllers[controller][action]))
    ...
    const pluginsActions = Object.keys(mwapi.plugins).reduce((acc, plugin) => {
      Object.keys(mwapi.plugins[plugin].controllers)
        .map(controller => {
          const actions = Object.keys(mwapi.plugins[plugin].controllers[controller])
            .filter(action => _.isFunction(mwapi.plugins[plugin].controllers[controller][action]))
    ...
    const permissions = await mwapi.query('permission', 'users-permissions').count();
    ...
    if (!_.isEqual(actions, currentActions) || permissions < 1) {
      ...
      // Retrieve roles
      const roles = await mwapi.query('role', 'users-permissions').find();
        ...
        roles.map(role =>
          Promise.all(
            toAdd
              ...
              .map(action => mwapi.query('permission', 'users-permissions')
                .addPermission(Object.assign(action, { role: role.id || role._id }))
              )
          )
        ),
        Promise.all(toRemove.map(action => mwapi.query('permission', 'users-permissions').removePermission(action)))
      );
  ...
  initialize: async function (cb) {
    const roles = await mwapi.query('role', 'users-permissions').count();
    ...
    await Promise.all([
      mwapi.query('role', 'users-permissions').create({
        name: 'Administrator',
  ...
  updateRole: async function (roleID, body) {
    const [role, root, guest] = await Promise.all([
      this.getRole(roleID, []),
      mwapi.query('role', 'users-permissions').findOne({ type: 'root' }, []),
      mwapi.query('role', 'users-permissions').findOne({ type: 'guest' }, [])
    ]);
      ...
      Object.keys(body.permissions[type].controllers).forEach(controller => {
        Object.keys(body.permissions[type].controllers[controller]).forEach(action => {
          ...
          if (_.differenceWith([bodyAction], [currentAction]).length > 0) {
            acc.push(mwapi.query('permission', 'users-permissions').update({
              role: roleID,
              type,
              controller,
              action: action.toLowerCase()
  ...
  updateUserRole: async (user, role) => {
    return mwapi.query('user', 'users-permissions').update({
      id: user._id || user.id
    }, {
      role: role.toString()
    });
  ...
  writeActions: (data) => {
    const actionsPath = path.join(mwapi.config.appPath, 'plugins', 'users-permissions', 'config', 'actions.json');

    try {
      // Stop auto reload.
      mwapi.reload.isReloading = false;
      // Rewrite actions.json file.
      fs.writeFileSync(actionsPath, JSON.stringify({ actions: data }), 'utf8');
      // Set value to AST to avoid restart.
      _.set(mwapi.plugins['users-permissions'], 'config.actions', data);
  ...
  syncSchema: async (cb) => {
    if (mwapi.plugins['users-permissions'].models.user.orm !== 'bookshelf') {
      return cb();
    }
    ...
    } = mwapi.plugins['users-permissions'].models;
    ...
    const hasTables = await Promise.all(Object.keys(details).map(name =>
      mwapi.connections[details[name].connection].schema.hasTable(details[name].tableName)
    ));
...
2️⃣  RESTART YOUR SERVER`)
      mwapi.log.warn(missingTables.concat(tablesToCreate).join(''));
    ...
    for (let index = 0; index < hasTables.length; ++index) {
      ...
      const attributes = {
        ...
        ..._.cloneDeep(mwapi.plugins['users-permissions'].models[currentModel].attributes)
      };
      ...
      const hasColumns = await Promise.all(columns.map(attribute =>
        mwapi.connections[details[currentModel].connection].schema.hasColumn(details[currentModel].tableName, attribute)
      ));
...
2️⃣  RESTART YOUR SERVER`)
      mwapi.log.warn(missingColumns.concat(tablesToAlter).join(''));
