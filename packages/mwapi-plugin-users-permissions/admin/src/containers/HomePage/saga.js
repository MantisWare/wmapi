  ...
  try {
    ...
    if (indexDataToDelete !== -1) {
      ...
      if (response.ok) {
        ...
        mwapi.notification.success('users-permissions.notification.success.delete');
      }
    }
  } catch(err) {
    mwapi.notification.error('users-permissions.notification.error.delete');
  }
}
...
export function* dataFetch(action) {
  try {
    ...
    console.log(err);
    mwapi.notification.error('users-permissions.notification.error.fetch');
  }
}
...
export function* submitData(action) {
  try {
    ...
    mwapi.notification.success('users-permissions.notification.success.submit');
  } catch(error) {
