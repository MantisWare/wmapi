...
export function* fetchUser(action) {
  try {
    ...
    mwapi.notification.error('users-permissions.notification.error.fetchUser');
  }
}
...
export function* permissionsGet() {
  try {
    const response = yield call(request, '/users-permissions/permissions', {
      method: 'GET',
      params: {
        lang: mwapi.currentLanguage,
      },
    });

    yield put(getPermissionsSucceeded(response));
  } catch(err) {
    mwapi.notification.error('users-permissions.EditPage.notification.permissions.error');
  }
}
...
export function* policiesGet() {
  try {
    ...
    mwapi.notification.error('users-permissions.EditPage.notification.policies.error');
  }
}
...
export function* roleGet(action) {
  try {
    ...
    mwapi.notification.error('users-permissions.EditPage.notification.role.error');
  }
}
