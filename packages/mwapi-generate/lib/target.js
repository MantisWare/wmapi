...
function parseTarget(target, scope, cb) {
  ...
  if (target.generator) {
    ...
    // try requiring `mwapi-generate-<module>` to get the core generator.
    if (!subGenerator && !module.match(/^mwapi-generate-/)) {
      try {
        if (process.mainModule.filename.indexOf('yarn') !== -1) {
          subGenerator = require(path.resolve(process.mainModule.paths[2], 'mwapi-generate-' + module));
        } else {
          subGenerator = require(path.resolve(process.mainModule.paths[1], 'mwapi-generate-' + module));
        }
      } catch (e1) {
        requireError = e1;
      }
