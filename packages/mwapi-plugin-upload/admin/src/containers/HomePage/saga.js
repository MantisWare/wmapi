...
function* dataDelete(action) {
  try {
    ...
    mwapi.notification.success('upload.notification.delete.success');
  } catch(err) {
...
function* uploadFiles(action) {
  try {
    ...
    if (newFiles.length > 1) {
      mwapi.notification.success({ id: 'upload.notification.dropFile.success' });
    } else {
      mwapi.notification.success({ id: 'upload.notification.dropFiles.success', values: { number: newFiles.length } });
    }
