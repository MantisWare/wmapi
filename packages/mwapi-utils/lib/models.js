...
module.exports = {
  ...
  getPK: function (collectionIdentity, collection, models) {
    if (_.isString(collectionIdentity)) {
      ...
      try {
        const GraphQLFunctions = require(path.resolve(mwapi.config.appPath, 'node_modules', 'mwapi-' + ORM, 'lib', 'utils'));

        if (!_.isUndefined(GraphQLFunctions)) {
          return GraphQLFunctions.getPK(collectionIdentity, collection, models || mwapi.models);
        }
      } catch (err) {
        return undefined;
      }
  ...
  getCount: function (collectionIdentity) {
    if (_.isString(collectionIdentity)) {
      ...
      try {
        const ORMFunctions = require(path.resolve(mwapi.config.appPath, 'node_modules', 'mwapi-' + ORM, 'lib', 'utils'));
  ...
  getNature: (association, key, models, currentModelName) => {
    try {
      ...
      if (_.isUndefined(models)) {
        models = association.plugin ? mwapi.plugins[association.plugin].models : mwapi.models;
      }
      ...
      if ((association.hasOwnProperty('collection') && association.collection === '*') || (association.hasOwnProperty('model') && association.model === '*')) {
        ...
        const flattenedPluginsModels = Object.keys(mwapi.plugins).reduce((acc, current) => {
          Object.keys(mwapi.plugins[current].models).forEach((model) => {
            acc[`${current}_${model}`] = mwapi.plugins[current].models[model];
          });

          return acc;
        }, {});

        const allModels = _.merge({}, mwapi.models, flattenedPluginsModels);
      ...
      mwapi.log.error(`Something went wrong in the model \`${_.upperFirst(currentModelName)}\` with the attribute \`${key}\``);
      mwapi.log.error(e);
      mwapi.stop();
    }
  },
  ...
  getORM: collectionIdentity => {
    return _.get(mwapi.models, collectionIdentity.toLowerCase() + '.orm');
  },
    ...
    try {
      ...
      if (globalName !== '*') {
        details = association.plugin ?
          _.get(mwapi.plugins, `${association.plugin}.models.${globalName}.attributes.${association.via}`, {}):
          _.get(mwapi.models, `${globalName}.attributes.${association.via}`, {});
      }
      ...
      if (association.hasOwnProperty('collection') && association.collection !== '*') {
        ...
        const pluginsModels = Object.keys(mwapi.plugins).reduce((acc, current) => {
          Object.keys(mwapi.plugins[current].models).forEach((entity) => {
            Object.keys(mwapi.plugins[current].models[entity].attributes).forEach((attribute) => {
              const attr = mwapi.plugins[current].models[entity].attributes[attribute];

              if (
                (attr.collection || attr.model || '').toLowerCase() === model.toLowerCase() &&
                mwapi.plugins[current].models[entity].globalId !== definition.globalId
              ) {
                acc.push(mwapi.plugins[current].models[entity].globalId);
              }
            });
          });
        ...
        const appModels = Object.keys(mwapi.models).reduce((acc, entity) => {
          Object.keys(mwapi.models[entity].attributes).forEach((attribute) => {
            const attr = mwapi.models[entity].attributes[attribute];

            if (
              (attr.collection || attr.model || '').toLowerCase() === model.toLowerCase() &&
              mwapi.models[entity].globalId !== definition.globalId
            ) {
              acc.push(mwapi.models[entity].globalId);
            }
          });
      ...
      mwapi.log.error(`Something went wrong in the model \`${_.upperFirst(model)}\` with the attribute \`${key}\``);
      mwapi.log.error(e);
      mwapi.stop();
    }
  },
  ...
  getVia: (attribute, association) => {
    return _.findKey(mwapi.models[association.model || association.collection].attributes, {via: attribute});
  },
  ...
  convertParams: (entity, params) => {
    ...
    const models = _.assign(_.clone(mwapi.models), Object.keys(mwapi.plugins).reduce((acc, current) => {
      _.assign(acc, _.get(mwapi.plugins[current], ['models'], {}));
      return acc;
    }, {}));
    ...
    const convertor = mwapi.hook[connector].load().getQueryParams;
    const convertParams = {
      where: {},
      sort: '',
      start: 0,
