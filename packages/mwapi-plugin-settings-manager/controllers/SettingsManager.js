...
module.exports = {
  menu: async ctx => {
    const Service = mwapi.plugins['settings-manager'].services.settingsmanager;
  ...
  databaseModel: async ctx => {
    const Service = mwapi.plugins['settings-manager'].services.settingsmanager;
    const env = mwapi.config.environment;
  ...
  createLanguage: async ctx => {
    ...
    const availableLanguages = mwapi.plugins['settings-manager'].services.languages;
    ...
    const filePath = path.join(mwapi.config.appPath, 'config', 'locales', `${name}.json`);
  ...
  createDatabase: async ctx => {
    ...
    if (_.isEmpty(_.keys(mwapi.config.environments[env].database.connections)) || _.isEmpty(mwapi.config.environments[env].database.defaultConnection)) {
  ...
  updateDatabase: async ctx => {
    ...
    if (params.database.connections) {
      const settings = _.assign(_.clone(mwapi.config.environments[env].database.connections[name].settings), params.database.connections[name].settings);
      params = _.assign(_.clone(mwapi.config.environments[env].database.connections[name]), params.database.connections[name]);
      params.settings = settings;
    }

    delete params.name;

    const connections = _.clone(mwapi.config.environments[env].database.connections);
    ...
    if (newName && newName !== name) {
      ...
      _.forEach(mwapi.models, (model, modelName) => {
    ...
    if (newName && newName !== name && mwapi.config.environments[env].database.defaultConnection === name) {
  ...
  deleteDatabase: async ctx => {
    ...
    if (mwapi.config.environments[env].database.defaultConnection === name) {
