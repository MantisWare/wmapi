...
const basename = mwapi.remoteURL.replace(window.location.origin, '');
    ...
    .then(plugins => {
      ...
      (plugins || []).forEach(plugin => {
        ...
        script.onerror = function (oError) {
          ...
          const url = new URL(`${mwapi.remoteURL}`);
...
/**
 * Public mwapi object exposed to the `window` object
 */

window.mwapi = Object.assign(window.mwapi || {}, {
  ...
  router: history,
  languages,
  currentLanguage: window.localStorage.getItem('mwapi-admin-language') ||  window.navigator.language ||  window.navigator.userLanguage || 'en',
  lockApp,
  unlockApp,
});
