...
const isDevelopmentMode = path.resolve(pwd.stdout).indexOf('mwapi-admin') !== -1;
...
if (isDevelopmentMode) {
  shell.exec(`cd ${path.resolve(appPath, 'admin')} && npm link mwapi-helper-plugin && npm link mwapi-utils`, {
    silent
  });
} else {
  shell.exec(`cd ${path.resolve(appPath, 'admin', 'node_modules', 'mwapi-helper-plugin')} && npm install`, {
    silent
  });
}
...
if (process.env.npm_config_plugins === 'true') {
  ...
  shell.ls('* -d', plugins).forEach(function (plugin) {
    ...
    if (isDevelopmentMode) {
      shell.exec(`cd ${path.resolve(plugins, plugin)} && npm link mwapi-helper-plugin`, {
        silent
      });
    } else {
      shell.exec(`cd ${path.resolve(plugins, plugin, 'node_modules', 'mwapi-helper-plugin')} && npm install`, {
        silent
      });
    }
