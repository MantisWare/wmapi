...
To extract the filters from an JavaScript object or a request, you need to call the [`mwapi.utils.models.convertParams` helper](../api-reference/reference.md#mwapiutils).
...
// Convert params.
const formattedParams = mwapi.utils.models.convertParams('user', params); // { limit: 20, sort: 'email' }
```
...
module.exports = {
  ...
  find: async (ctx) => {
    // Convert params.
    const formattedParams = mwapi.utils.models.convertParams('user', ctx.request.query);
