...
Use the CLI, and run the following command `mwapi generate:model user firstname:string lastname:string`. Read the [CLI documentation](../cli/CLI.md) for more informations.
...
> Note: when you create a new API using the CLI (`mwapi generate:api <name>`), a model is automatically created.
...
If you're using MongoDB as a database, you don't need to do anything. Everything is natively handled by mwapi. However, to implement a polymorphic relationship with SQL databases, you need to create two tables.
...
> Note: If you've overrided the default table name given by mwapi by using the `collectionName` attribute. Use the value set in the `collectionName` to name the table.
...
module.exports = {
 ...
 beforeCreate: async (model) => {
   // Hash password.
   const passwordHashed = await mwapi.api.user.services.user.hashPassword(this.password);
...
module.exports = {
  ...
  beforeCreate: async (model, attrs, options) => {
      // Hash password.
      const passwordHashed = await mwapi.api.user.services.user.hashPassword(model.attributes.password);
