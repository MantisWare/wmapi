...
A plugin can also use a globally exposed policy in the current mwapi project.
...
mwapi supports multiple ORMs in order to let the users choose the database management system that suits their needs. Hence, each plugin must be compatible with at least one ORM. Each plugin contains a folder named `queries` in `./plugins/**/api/queries`. A folder must be created for each ORM (eg. `mongoose`) with a file named `mongoose.js` which exports the Mongoose ORM related queries.

The queries are accessible through the `mwapi.query()` method, which automatically contains the queries according to the ORM used by the model.
...
module.exports = {
  getUsers: async () => {
    ...
    const users = await mwapi.query('User').getUsers({ limit, sort });
...
module.exports = {
  getAll: async function (params) {
    ...
    // ex: mwapi.query('User').getAll(), this will be equal to the User Mongoose model.
...
module.exports = {
  getAll: async function (params) {
    ...
    // ex: mwapi.query('User').getAll(), this will be equal to the User Bookshelf model.
...
module.exports = {
  getUsers: async () => {
    ...
    const users = await mwapi.query('User').getAll({ limit, sort });
