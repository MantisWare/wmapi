...
Add custom configurations to the project. The content of this file is available through the `mwapi.config` object.
...
These configurations are accessible through `mwapi.config.backendURL` and `mwapi.config.mainColor`.

***

## Language

As described in the [i18n documentation](../plugin-development/frontend-development.md#i18n), mwapi includes an internationalization system. This is especially useful to translate API messages (errors, etc.).
...
> Note: You can access to the config of the current environment through `mwapi.config.currentEnvironment`.
...
**Path —** `./config/environments/**/database.json`.
 ...
 - `connections` List of all available connections.
   - `default`
     - `connector` (string): Connector used by the current connection. Default value: `mwapi-mongoose`.
     - `client` (string): Client used to store session. Default value: `cookie`.
     - `key` (string): Cookie key name. Default value: `mwapi.sid`
...
**Path —** `./config/environments/**/request.json`.
 ...
 - `logger`
   ...
   - `exposeInContext` (boolean): Expose logger in context so it can be used through `mwapi.log.info(‘my log’)`. Default value: `true`.
